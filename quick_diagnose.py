...
pending_missing = db.query(Media).filter(Media.missing_scans_count > 0).count()

print(f"Total no banco: {total}")
print(f"Visíveis (não-duplicatas): {non_dup}")
print(f"Duplicatas: {duplicates}")
print(f"Missing (deletados): {missing}")
print(f"Pendente (aguardando confirmação): {pending_missing}")
...
print(f"   - {missing} foram marcadas como missing (deletadas)")
print(f"   - {pending_missing} aguardando confirmação (HD desplugado?)")
print(f"   - {not_organized} não estão organizadas")
print(f"\n💡 Se HD foi desplugado e reconectado, rode: curl -X POST http://localhost:8000/api/jobs/sync")
